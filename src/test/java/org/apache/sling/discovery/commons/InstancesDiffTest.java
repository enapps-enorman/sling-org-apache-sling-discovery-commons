...
public class InstancesDiffTest {
    ...
    public void testAddedWithoutIntersection() throws Exception {
        InstancesDiff diff = new InstancesDiff(
                Collections.singletonList(new Instance("one")), Collections.singletonList(new Instance("two")));
    ...
    public void testAddedWithIntersection() throws Exception {
        InstancesDiff diff = new InstancesDiff(
                Arrays.asList(new Instance("one"), new Instance("two")),
    ...
    public void testAll() throws Exception {
        InstancesDiff diff = new InstancesDiff(
                Collections.singletonList(new Instance("one")),
                Arrays.asList(new Instance("two"), new Instance("three")));
    ...
    public void testAllRetainedCollection() throws Exception {
        ...
        InstancesDiff diff =
                new InstancesDiff(Collections.singletonList(oldInstance), Collections.singletonList(newInstance));
    ...
    public void testRetainedByProperties() throws Exception {
        InstancesDiff diff = new InstancesDiff(
                Arrays.asList(
                        new Instance("one", Collections.singletonMap("p1", "v1")),
                        new Instance("two", Collections.singletonMap("p1", "v1"))),
                Arrays.asList(
                        new Instance("one", Collections.singletonMap("p1", "v2")),
                        new Instance("two", Collections.singletonMap("p1", "v1"))));
        TestCase.assertEquals(1, diff.retained(true, false).get().size());
        TestCase.assertEquals(
                "two", diff.retained(true, false).get().iterator().next().getSlingId());
        TestCase.assertEquals(1, diff.retained(true, true).get().size());
        TestCase.assertEquals(
                "one", diff.retained(true, true).get().iterator().next().getSlingId());
    }
    ...
    public void testEmptyResult() throws Exception {
        Collection<InstanceDescription> instances = new InstancesDiff(
                        Arrays.asList(new Instance("one"), new Instance("two")), empty())
                .all(true)
                .filterWith(new InstanceFilter() {
    ...
    public void testIsLeader() throws Exception {
        Collection<InstanceDescription> instances = new InstancesDiff(
                        Arrays.asList(
                                ...
                                new Instance("two", false, false, Collections.<String, String>emptyMap(), "viewId")),
                        empty())
                .all(true)
                .isLeader()
                .get();
    ...
    public void testIsLocal() throws Exception {
        Collection<InstanceDescription> instances = new InstancesDiff(
                        Arrays.asList(
                                new Instance("one", true, false, Collections.<String, String>emptyMap(), "viewId"),
                                new Instance("two", false, true, Collections.<String, String>emptyMap(), "viewId")),
                        empty())
                .all(true)
                .isLocal()
                .get();
