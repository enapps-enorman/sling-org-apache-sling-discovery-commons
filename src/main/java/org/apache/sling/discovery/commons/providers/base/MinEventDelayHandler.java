...
class MinEventDelayHandler {
    ...
    MinEventDelayHandler(
            ViewStateManagerImpl viewStateManager,
            Lock lock,
        ...
        if (minEventDelaySecs <= 0) {
            throw new IllegalArgumentException(
                    "minEventDelaySecs must be greater than 0 (is " + minEventDelaySecs + ")");
        }
    ...
    boolean handlesNewView(BaseTopologyView newView) {
        if (isDelaying) {
            ...
            logSilencer.infoOrDebug(
                    "handlesNewView-" + newView.getLocalClusterSyncTokenId(),
    ...
    private boolean triggerAsyncDelaying(final BaseTopologyView newView) {
        ...
        final boolean triggered = runAfter(
                minEventDelaySecs /*seconds*/, new Runnable() {
                    ...
                    public void run() {
                        ...
                        try {
                            if (cancelCnt != validCancelCnt) {
                                logSilencer.infoOrDebug(
                                        "asyncDelay.run-cancel-" + newView.getLocalClusterSyncTokenId(),
                                        "asyncDelay.run: got cancelled (validCancelCnt=" + validCancelCnt
                                                + ", cancelCnt=" + cancelCnt + "), quitting.");
                                return;
                            }
                            ...
                            if (!(t instanceof BaseTopologyView)) {
                                logger.error(
                                        "asyncDelay.run: done delaying. topology not of type BaseTopologyView: " + t);
                            ...
                            if (topology.isCurrent()) {
                                logSilencer.infoOrDebug(
                                        "asyncDelay.run-done-" + newView.getLocalClusterSyncTokenId(),
        ...
        logSilencer.infoOrDebug(
                "triggerAsyncDelaying",
                "triggerAsyncDelaying: asynch delaying of " + minEventDelaySecs + " triggered: " + triggered);
        if (triggered) {
            isDelaying = true;
        }
        return triggered;
    ...
    private final void assertCorrectThreadPool() {
        if (!Thread.currentThread().getName().contains("sling-discovery")) {
            logger.warn(
                    "assertCorrectThreadPool : not running as part of 'discovery' thread pool."
