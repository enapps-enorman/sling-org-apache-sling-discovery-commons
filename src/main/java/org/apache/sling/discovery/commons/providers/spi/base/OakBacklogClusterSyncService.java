 ...
 */
@Component(
        service = {ClusterSyncService.class, OakBacklogClusterSyncService.class},
...
public class OakBacklogClusterSyncService extends AbstractServiceWithBackgroundCheck implements ClusterSyncService {
    ...
    public static OakBacklogClusterSyncService testConstructorAndActivate(
        ...
        OakBacklogClusterSyncService service =
                testConstructor(commonsConfig, idMapService, settingsService, resourceResolverFactory);
        service.activate();
        return service;
    }
    ...
    private void waitWhileBacklog(final BaseTopologyView view, final Runnable runnable) {
        ...
        startBackgroundCheck(
                "OakBacklogClusterSyncService-backlog-waiting-" + view.getLocalClusterSyncTokenId(),
                new BackgroundCheck() {
                        ...
                        try {
                            if (!idMapService.isInitialized()) {
                                logSilencer.infoOrDebug(
                                        "waitWhileBacklog-" + view.toShortString(),
                            ...
                            consistencyHistory.addHistoryEntry(
                                    view, "got Exception while initializing idMapService (" + e + ")");
                            return false;
                        }
                ...
                runnable,
                getCommonsConfig().getClusterSyncServiceTimeoutMillis(),
                getCommonsConfig().getClusterSyncServiceIntervalMillis());
    }
    ...
    private BacklogStatus getBacklogStatus(BaseTopologyView view) {
        ...
        try {
            ...
            if (deactivatingIds.length != 0) {
                logSilencer.infoOrDebug(
                        "getBacklogStatus-hasBacklog-" + view.toShortString(),
            ...
            for (int i = 0; i < activeIds.length; i++) {
                ...
                if (slingId == null) {
                    logSilencer.infoOrDebug(
                            "getBacklogStatus-undefined-" + view.toShortString(),
                ...
                if (!slingIds.contains(slingId)) {
                    logSilencer.infoOrDebug(
                            "getBacklogStatus-hasBacklog-" + view.toShortString(),
                            "getBacklogStatus: active instance's (" + activeId + ") slingId (" + slingId
                                    + ") not found in cluster (" + cluster + ")");
                    return BacklogStatus.HAS_BACKLOG;
                }
            }

            logSilencer.infoOrDebug(
                    "getBacklogStatus-" + view.toShortString(), "getBacklogStatus: no backlog (anymore)");
